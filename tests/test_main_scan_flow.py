...
main.py: ask_port_scope(), ask_service_detection(),
...
# ---------------------------------------------------------------------------
# ask_port_scope() / ask_service_detection()
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# get_scan_config_from_user() - composing both questions, with/without a
# previous ScanConfig supplying the defaults
# ---------------------------------------------------------------------------
...
def test_get_scan_config_from_user_uses_previous_config_as_default(monkeypatch):
    previous = scan.ScanConfig(
        port_scope=scan.PortScope.ALL_TCP, service_detection=False
    )
    # Blank input on both questions should preserve the previous
...
def test_selecting_non_default_options_and_accepting(monkeypatch):
    ...
    # Consent: "y".
    queued_input(monkeypatch, ["3", "n", "y"])
...
def test_declining_and_reconfiguring_preserves_previous_selection(monkeypatch):
    # Pass 1: choose ALL_TCP, service detection off, then decline ("2").
    # Pass 2: accept both defaults, which must now be the previous
    # answers (ALL_TCP, disabled) rather than TOP_1000/enabled.
    # Consent: "1".
    queued_input(monkeypatch, ["3", "n", "2", "", "", "1"])
    config = main.get_confirmed_scan_config(TARGET)
    assert config == scan.ScanConfig(
        port_scope=scan.PortScope.ALL_TCP, service_detection=False
    )
...
def test_declining_and_deliberately_changing_one_answer(monkeypatch):
    # Pass 1: ALL_TCP, detection enabled (default), then decline ("2").
    # Pass 2: keep port scope (blank = preserved ALL_TCP), but
    # deliberately turn detection off this time. Consent: "1".
